(function ($) {
  ...
  $(document).ready(function () {
    try {
      $(document)
        .find("select:not(.noselect2):not(.inmodal)")
        .each(function () {
      ...
      $(document)
        .find("select.inmodal:not(.noselect2)")
        .each(function () {
          $(this).select2({
  ...
  if (tinymceSelector.length > 0) {
    tinymce.init({
      ...
      menubar: true,
      plugins:
        "codesample link media image code fullscreen table autolink advlist lists autoresize emoticons wordcount",
      toolbar: [
  ...
  $(document).ready(function () {
    ...
    if (datatableAjaxElement.length > 0) {
      try {
        const table = datatableAjaxElement
          .on("init.dt", function () {
            $(
              ".student_lists_table .dt-container .row:first-child > div:first-child"
            ).append(`
          ...
          .DataTable({
            ...
            language: {
              emptyTable:
                datatableAjaxElement.data("nodata") ?? "No data available",
            },
            processing: true,
        ...
        $("#campus-filter, #program-filter, #department-filter").change(
          function () {
            table.ajax.reload();
          }
        );
    ...
    function enable_or_disable_buttons() {
      console.log("Selected IDs:", selected_student_ids);

      $("#student_create_section").prop(
        "disabled",
        selected_student_ids.length === 0
      );
      $("#delete-btn").prop("disabled", selected_student_ids.length === 0);
    }

    function updateStudentIds() {
      selected_student_ids = [
        ...new Set(
          $(".student_lists_table tbody input[type=checkbox]:checked")
            .map(function () {
              return $(this).val();
            })
            .get()
    ...
    $(document).on(
      "change",
      ".student_lists_table tbody input[type=checkbox]",
      function () {
        updateStudentIds();
      }
    );

    // Attach event listener for the "Select All" checkbox
    $("#allCheckbox").on("change", function () {
      const isChecked = $(this).is(":checked");
      $(".student_lists_table tbody input[type=checkbox]").prop(
        "checked",
        isChecked
      );
      updateStudentIds();
    ...
    $(document).on("submit", "#update_sections", function (e) {
      ...
      $("#student_ids_input").val(selected_student_ids.join(",")); // Use only for section update
                        ...
                        <div class="image_fill_placeholder mt-2" style="border: 2px solid #eeeeee;border-radius: 10px;padding: 10px;">
                            ...
                            <div class="file_card" style="display: flex;align-items: center;gap: 10px;">
                                ...
                                <div class="file_info">
                                    ...
                                    <div class="file_size" style="color:#ccc;">Size: ${
                                      file.size || "Unknown"
                                    }</div>
                                </div>
                            </div>
  ...
  function handle_about_us_change() {
    ...
    if (about_us.length > 0) {
      ...
      if (value === "OTHER") {
        ...
        $(document)
          .find("#about_us_other_container textarea")
          .prop("required", true);
      } else {
  ...
  function handle_whychoose_change() {
    ...
    if (why_us.length > 0) {
      ...
      if (value === "OTHER") {
        ...
        $(document)
          .find("#why_us_other_container textarea")
          .prop("required", true);
      } else {
  ...
  $(document).on("submit", "#add-ids-form", function (e) {
    ...
    const rowButton = $(document).find(
      `.addIdsModal[data-studentid="${student_id}"]`
    );
  ...
  $(document).on("click", ".sameAsPermanent", function () {
    ...
    const contactnumber = $(document)
      .find("[name='permanent-contact_number']")
      .val();
  ...
  function handle_e_book_system() {
    ...
    const available_quantity = $(document)
      .find("#available_quantity")
      .closest("div");
    const image_picker_container = $(document)
      .find(".image_picker_container")
      .parent();
  ...
  $(document).on("click", ".updateFeeModal", function () {
    ...
    $(document)
      .find('#paymentForm input[name="student_id"]')
      .val(studentId)
      .trigger("change");
  ...
  // On BS date change, update AD field
  $("#date_of_birth_in_bs").on("dateChange", function (event) {
    const date = event.datePickerData.adDate;
    ...
    $("#date_of_birth_in_ad").val(formatDate);
  });

  // On AD change, update BS field
  $("#date_of_birth_in_ad").on("change", function () {
    const date = new Date($(this).val());
    $("#date_of_birth_in_bs").setEnglishDate(date);
  });
})(jQuery);
