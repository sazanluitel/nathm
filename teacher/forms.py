...
class TeacherAddForm:
    ...
    def save(self, commit=True):
        with transaction.atomic():
            ...
            personal_info.permanent_address = address_info  # Adjust as necessary
            if commit:
                personal_info.save()
            ...
            instance = Teacher()
            instance.user = user
            instance.campus = self.teacher_form.cleaned_data.get('campus')
            instance.department = self.teacher_form.cleaned_data.get('department')
            instance.program = self.teacher_form.cleaned_data.get('program')
            instance.english_test = english_test
            instance.shift = self.teacher_form.cleaned_data.get('shift')
